...
class MovieDAO:
    ...
    def get_twenty(self, page):
        return self.session.query(Movie).limit(12).offset((page-1)*12)

    def get_newest(self):
        return self.session.query(Movie).order_by(desc(Movie.year)).all()

    def get_twenty_newest(self, page):
        return self.session.query(Movie).order_by(desc(Movie.year)).limit(12).offset(page-1*12)
